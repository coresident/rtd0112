/**
 * \file
 * \brief Ray Tracing Components for RTD Integration
 * 
...
struct Float3ToBevTransform {
    ...
    __host__ __device__ float3 transform(const float3& point) const {
...
struct DensityAndSpTracerParams {
    ...
    __host__ __device__ float3 getStart(unsigned int x, unsigned int y) const {
        return make_float3(
    ...
    __host__ __device__ float3 getInc(unsigned int x, unsigned int y) const {
        return stepInc;
    }
    
    __host__ __device__ float stepLen(unsigned int x, unsigned int y) const {
        return stepLength;
    }
