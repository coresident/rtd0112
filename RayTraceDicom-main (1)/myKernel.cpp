...
void cudaWrapperProtons(HostPinnedImage3D<float>* const imVol, HostPinnedImage3D<float>* const doseVol, const std::vector<BeamSettings> beams, const EnergyStruct iddData, std::ostream &outStream) {
    ...
    weq::create3DTexture<float>
        (
            (float*)imCopyParams.srcPtr.ptr, //host data
            &devImVolArr, //cuda array
            &imVolTex, //texture object
            weq::LINEAR,
            weq::R,
            static_cast<size_t>(imVol->getDims().x), static_cast<size_t>(imVol->getDims().y), static_cast<size_t>(imVol->getDims().z),
            0 //GPU id
        );

    cudaArray_t devCumulIddArr;
    cudaTextureObject_t cumulIddTex;
    weq::create2DTexture(
        &iddData.ciddMatrix[0],
        &devCumulIddArr,
        &cumulIddTex,
        weq::LINEAR,
        weq::R,
        static_cast<size_t>(iddData.nEnergySamples),
        static_cast<size_t>(iddData.nEnergies),
        0
    );
    
    cudaArray_t devDensityArr;
    cudaTextureObject_t densityTex;
    weq::create1DTexture(
        &iddData.densityVector[0],
        &devDensityArr,
        &densityTex,
        weq::LINEAR,
        weq::R,
        static_cast<size_t>(iddData.nDensitySamples),
        0
    );
 
    
    cudaArray_t devStoppingPowerArr;
    cudaTextureObject_t stoppingPowerTex;
        weq::create1DTexture(
            ...
            weq::LINEAR,
            weq::R,
            static_cast<size_t>(iddData.nSpSamples),
            0
        );
    ...
    cudaTextureObject_t rRadiationLengthTex;
        weq::create1DTexture(
            &iddData.rRlVector[0],
            &devReciprocalRadiationLengthArr,
            &rRadiationLengthTex,
            weq::LINEAR,
            weq::R,
            static_cast<size_t>(iddData.nRRlSamples),
            0
        );
    ...
    for (unsigned int beamNo=0; beamNo<beams.size(); ++beamNo)
    {
        ...
        weq::create3DTexture<float>
            (
                (float*)primDoseCopyParams.srcPtr.ptr, //host data
                &devBevPrimDoseArr, //cuda array
                &bevPrimDoseTex, //texture object
                weq::LINEAR,
                weq::R,
                static_cast<size_t>(bevPrimDoseDim.x), static_cast<size_t>(bevPrimDoseDim.y), static_cast<size_t>(beamFirstCalculatedPassive-beamFirstInside),
                0 //GPU id
            );
